...
class InicioController extends Controller
{
    ...
    public function DatosUpdate(Request $request)
    {
        ...
        if (auth()->user()->email != request('email')) {
            if (isset($datos["passwordN"])) {
                $datos = request()->validate([
                    ...
                    'passwordN' => ['string', 'min:8', 'required']
                ]);
            }else{
                $datos = request()->validate([
                    'name' => ['required', 'string', 'max:255'],
        ...
        if (isset($datos["passwordN"])) {
            ...
            DB::table('users')->where('id',auth()->user()->id)->update(['name' =>$datos["name"], 'email' =>$datos["email"], 'documento' =>$datos["documento"],'telefono' =>$datos["telefono"], 'password' => Hash::make($datos["passwordN"])]);
        }
