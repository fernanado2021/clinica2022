...
@elseif(session('Nborrar') == 'Nb')
   <script>
      Swal.fire(
                    'Consultorio no se puede  Eliminar ',
                    'Tiene un doctor asignado ha este consultorio',
                    'info'
                  )
    </script>
@elseif(session('BorrarC') == 'BC')
   <script>
      Swal.fire(
...
<script >
  $('.table').on('click', '.EliminarConsultorio',function(){
    var Did = $(this).attr('Did');
       Swal.fire({
            ...
            if(result.isConfirmed){
                window.location = "borrar-Consultorio";
            }
       })
  })
</script>
